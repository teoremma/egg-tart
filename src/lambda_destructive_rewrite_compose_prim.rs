...
impl Lambda {
    ...
    fn increment_id(&self, increment: usize) -> Self {
        match self {
            ...
            Lambda::Add([id1, id2]) => {
                Lambda::Add([increment_id(*id1, increment), increment_id(*id2, increment)])
            }
            Lambda::Eq([id1, id2]) => {
                Lambda::Eq([increment_id(*id1, increment), increment_id(*id2, increment)])
            }
            Lambda::App([id1, id2]) => {
                Lambda::App([increment_id(*id1, increment), increment_id(*id2, increment)])
            }
            Lambda::Lambda([id1, id2]) => {
                Lambda::Lambda([increment_id(*id1, increment), increment_id(*id2, increment)])
            }
            Lambda::Let([id1, id2, id3]) => Lambda::Let([
                increment_id(*id1, increment),
                increment_id(*id2, increment),
                increment_id(*id3, increment),
            ]),
            Lambda::Fix([id1, id2]) => {
                Lambda::Fix([increment_id(*id1, increment), increment_id(*id2, increment)])
            }
            Lambda::If([id1, id2, id3]) => Lambda::If([
                increment_id(*id1, increment),
                increment_id(*id2, increment),
                increment_id(*id3, increment),
            ]),
            Lambda::Map([id1]) => Lambda::Map([increment_id(*id1, increment)]),
            Lambda::Compose([id1, id2]) => {
                Lambda::Compose([increment_id(*id1, increment), increment_id(*id2, increment)])
            }
...
impl Analysis<Lambda> for LambdaAnalysis {
    ...
    fn make(egraph: &EGraph, enode: &Lambda) -> Data {
        ...
        Data {
            constant,
            free,
            previous_rewrites: HashSet::default(),
        }
        // Data { constant, free }
    }
...
fn is_not_same_var_and_free_in_body(
    v1: Var,
    v2: Var,
    body: Var,
...
fn is_free_in_vars(v: Var, vars: Vec<Var>) -> impl Fn(&mut EGraph, Id, &Subst) -> bool {
    move |egraph, _, subst| {
        vars.iter()
            .any(|body| egraph[subst[*body]].data.free.contains(&subst[v]))
    }
}
...
impl Applier<Lambda, LambdaAnalysis> for DestructiveRewrite {
    fn apply_one(
        ...
        if prune_enodes_matching(
            egraph,
            &self.original_pattern.ast,
            subst,
            &eclass,
            rule_name,
        ) {
...
fn prune_enodes_matching(
    egraph: &mut egg::EGraph<Lambda, LambdaAnalysis>,
    rec_expr: &RecExpr<ENodeOrVar<Lambda>>,
    subst: &Subst,
    eclass: &Id,
    rule_name: Symbol,
) -> bool {
...
fn any_enode_in_eclass_matches(
    egraph: &egg::EGraph<Lambda, LambdaAnalysis>,
    rec_expr: &RecExpr<ENodeOrVar<Lambda>>,
    rec_expr_id: &Id,
    subst: &Subst,
...
fn dr_simple_example() {
    ...
    // Id 1
    let start = "(app (lam x (app (var x) (app (var y) (var z)))) 1)"
        .parse()
        .unwrap();
...
#[test]
fn lambda_dr_map_fusion_many_compose_prim() {
    let range = 1..50;
    for n in range {
